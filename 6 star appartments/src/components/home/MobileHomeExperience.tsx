...
export const MobileHomeExperience: React.FC = () => {
  ...
  const availableCategories = categories.filter(category => apartments.some(apartment => apartment.category_id === category.id)).slice(0, 4);
  const leadApartment = featured[0];
  ...
  return (
    <div className="md:hidden bg-[#fbfaf7] pb-20 text-[#102033]">
      <section className="relative -mt-1 overflow-hidden bg-[#0a1626] px-4 pb-4 pt-4">
        ...
        <div className="relative z-10 mt-4 rounded-xl bg-white p-2.5 shadow-xl">
          <div className="mb-2 flex items-center justify-between border-b border-[#ebe5da] pb-2">
            ...
            <span className="text-[9px] font-semibold text-slate-500">{leadApartment?.category_name || 'Choose a residence'}</span>
          </div>
          ...
          <div className="mt-2 grid grid-cols-3 gap-1.5 text-[9px]">
            <div className="rounded-lg bg-[#f8f6f1] p-2"><span className="block uppercase text-slate-400">Dates</span><strong>Select dates</strong></div>
            <div className="rounded-lg bg-[#f8f6f1] p-2"><span className="block uppercase text-slate-400">Residents</span><strong>{leadApartment ? `Up to ${leadApartment.guest_capacity} guests` : 'Select guests'}</strong></div>
            <div className="rounded-lg bg-[#f8f6f1] p-2"><span className="block uppercase text-slate-400">View</span><strong>{leadApartment?.view_type || 'Choose a view'}</strong></div>
          </div>
      ...
      <section className="px-4 pt-5">
        ...
        {availableCategories.length > 0 ? <div className="mt-3 grid grid-cols-4 gap-1.5">
        ...
        </div> : <p className="mt-3 rounded-lg border border-[#e5dfd3] bg-white p-3 text-[10px] text-slate-500">No residence categories are currently available.</p>}
      </section>
      ...
      <section className="px-4 pt-7">
        ...
        <div className="mt-3 space-y-4">
          {featured.length > 0 ? featured.map(apartment => <article key={apartment.id} className="overflow-hidden rounded-xl border border-[#e2dbce] bg-white shadow-sm"><Link to={`/apartments/${apartment.id}`}><div className="relative"><img src={resolveSafeImagePath(apartment.images[0])} alt={apartment.title} className="h-40 w-full object-cover" width={1280} height={800} /><span className="absolute left-2 top-2 rounded-md bg-[#0b1828]/85 px-2 py-1 text-[8px] font-bold uppercase text-[#e5c47e]">{apartment.location || apartment.category_name || 'Serviced residence'}</span><span className="absolute right-2 top-2 rounded-md bg-white/90 px-2 py-1 text-[8px] font-bold text-[#152439]">{apartment.status}</span></div></Link><div className="p-3"><div className="flex items-start justify-between gap-3"><div><h3 className="font-heading text-base font-bold leading-tight">{apartment.title}</h3><p className="mt-1 text-[9px] text-slate-500">{apartment.view_type}</p></div><div className="text-right"><strong className="font-heading text-lg">{formatCurrency(apartment.price)}</strong><span className="block text-[8px] text-slate-400">/ {apartment.price_type}</span></div></div><div className="mt-3 grid grid-cols-3 border-y border-[#eee8dd] py-2 text-[8px] text-slate-500"><span>{apartment.bedrooms} BEDROOMS</span><span>{apartment.bathrooms} BATHROOMS</span><span>{apartment.area_sqft} SQ FT</span></div><div className="mt-3 flex gap-2"><Link to={`/apartments/${apartment.id}`} className="flex-1 rounded-md bg-[#101d2e] py-2 text-center text-[9px] font-bold text-white">View suite</Link><Link to="/quote" className="rounded-md border border-[#dfd6c7] px-3 py-2 text-[9px] font-bold">Quote</Link></div></div></article>) : <p className="rounded-lg border border-[#e5dfd3] bg-white p-4 text-[10px] text-slate-500">No suites are currently available.</p>}
        </div>
      </section>
      ...
      <section className="px-4 py-6"><p className="text-[8px] font-bold uppercase tracking-[0.2em] text-[#b08b44]">The Centaurus Complex</p><h2 className="font-heading text-base font-extrabold">Locations & Offices</h2><div className="mt-3 grid grid-cols-2 gap-2">{settings.showrooms.length > 0 ? settings.showrooms.slice(0, 4).map(showroom => <a href={showroom.map_url} target="_blank" rel="noreferrer" key={showroom.name} className="relative overflow-hidden rounded-lg"><img src={resolveSafeImagePath(featured[0]?.images[0] || '/images/background image.jpeg')} alt={showroom.name} className="h-20 w-full object-cover brightness-75" width={500} height={300} /><span className="absolute bottom-2 left-2 right-2 text-[9px] font-bold text-white">{showroom.name}</span></a>) : <p className="col-span-2 rounded-lg border border-[#e5dfd3] bg-white p-3 text-[10px] text-slate-500">Location details are currently unavailable.</p>}</div></section>
